...
//對輸入和輸出net 有排序過
//這個版本能處理N input gate，完成寫檔，不從gate name抓編號，且會自動用是否是"/"抓output pin與input pin
//如果只有OUT1，必然是Output，只有IN1、IN2等，必然是input，剩下有OUT也有IN，就是wire
//嘗試更穩的讀檔方式，主要是修改在read_file()

using namespace std;

struct net_gate_relation{   //儲存gat5/IN1的對應關係
    int gate_id;            //幫gate編號方便net去對應
    string role;            //OUT1 or IN1 or IN2 or IN3 or IN4等等
};
...
struct gate_node{
    string gate_type;  
    int gate_id;                    //依然按照順序幫gate編號(從0開始)，為了方便net去對應
...
bool check_slash(string input_str){
    if (input_str.find("/") != string::npos)return true;
    else    return false;
}
...
void construct_net_vector(vector<struct net_node>&net_vector, vector<string>& net_line, vector<string>& pin_line,unordered_map<string,int> gate_name_to_id_hash_table){
    for(int i=0;i<net_line.size();i++){
        ...
        int IN_count=0;
        int OUT_count=0;
        for(int i=1;i<pin_line_vec.size();i++){                 //index=0的地方一定是"PIN"
            //要有/才能切
            if(check_slash(pin_line_vec[i])){
                ...
                if(relation.role.substr(0,2)=="IN")IN_count++;
                else OUT_count++;
            }
        }
        if(OUT_count==0&&IN_count>0)my_net_node.role="input";
        else if(OUT_count>0&&IN_count==0)my_net_node.role="output";
        else my_net_node.role="wire";
...
class ouput_object {
    public: //在main中也可以被access
        ...
        ouput_object(vector<struct net_node>net_vector,vector<string> gate_line){
            ...
            sort(input_net_id.begin(),input_net_id.end());
            sort(output_net_id.begin(),output_net_id.end());
            sort(wire_net_id.begin(), wire_net_id.end());
...
string trim(const string str) {
    ...
    if (first == string::npos) return ""; // 全部是空白
...
void read_file(string inputFilename, string& first_line, vector<string>& gate_line, vector<string>& net_line, vector<string>& pin_line){
    ...
    while (getline(inputFile, line)) {
        ...
        if (!found_first_line&&!trimmed_line.empty()) {
            first_line = line;
            found_first_line = true;
        } 
        else {
            if (!trimmed_line.empty()){             // 如果把空格都移除後不為空
                if (line.find("Inst") == 0) {
                    gate_line.push_back(line);
                } else if (line.find("PIN") == 0) {
                    pin_line.push_back(line);
                } else if (line.find("NET") == 0) {
                    net_line.push_back(line);
                }
            }
        }
    }
...
string module_declaration(string module_name,vector<int> input_net_id,vector<int> output_net_id){
    ...
    vector<int> merged_vec(input_net_id.size() + output_net_id.size());
    merge(input_net_id.begin(),input_net_id.end(),output_net_id.begin(),output_net_id.end(),merged_vec.begin());

    for(int i=0;i<merged_vec.size();i++){
        ans=ans+"N"+to_string(merged_vec[i]);
        if(i!=merged_vec.size()-1)ans+=",";//如果不是最後的net要記得加逗號
    }
    ans+=");";
    return ans;
}
...
void PA1(string inputFilename,string outputFilename){
    ...
    read_file(inputFilename,first_line,gate_line,net_line,pin_line);
    
    construct_gate_name_to_id_hash_table(gate_line,gate_name_to_id_hash_table);
    construct_net_vector(net_vector,net_line,pin_line,gate_name_to_id_hash_table);  //建構net與gate和net的對應關係
    ...
    write_file(outputFilename,ouput_object_PA1,first_line);
}

int main(int argc, char *argv[]) {
    if (argc != 3) {
        cerr << "Input Command Wrong, use ./StudID_PA1.out <inputFilename> <outputFilename>\n";
        return 1;
    }
    string inputFilename = argv[1];     //讀取第一個參數作為輸入檔案名稱
    string outputFilename = argv[2];    //讀取第二個參數作為輸出檔案名稱
    PA1(inputFilename,outputFilename);
    return 0;
}
