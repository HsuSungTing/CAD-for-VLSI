...
struct gate_node{
    string gate_type;  
    int gate_id;
    vector<int> gate_IO_vector{0,0,0};//分別是OUT1的net_id, IN1的net_id, IN2的net_id 
};
...
void construct_gate_vector(vector<struct gate_node>&gate_vector,vector<struct net_node>net_vector,vector<string> gate_line){
    ...
    for(int i=0;i<net_vector.size();i++){
        ...
        for(int j=0;j<net_vector[i].relation_vector.size();j++){    //因為gate_id是連續的才能直接把gate_id當gate_vector的index
            if(net_vector[i].relation_vector[j].role=="OUT1"){
                gate_vector[net_vector[i].relation_vector[j].gate_id-1].gate_IO_vector[0]=net_vector[i].net_id;
            }
            else if(net_vector[i].relation_vector[j].role=="IN1"){
                gate_vector[net_vector[i].relation_vector[j].gate_id-1].gate_IO_vector[1]=net_vector[i].net_id;
            }
            else if(net_vector[i].relation_vector[j].role=="IN2"){
                gate_vector[net_vector[i].relation_vector[j].gate_id-1].gate_IO_vector[2]=net_vector[i].net_id;
            }
        }
    }
}
...
string gate_declaration(struct gate_node my_gate_node){
    string ans=my_gate_node.gate_type+" ";
    ans=ans+to_uppercase(my_gate_node.gate_type)+"2_"+to_string(my_gate_node.gate_id)+" (";
