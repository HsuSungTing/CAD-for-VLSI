...
struct cell_node{    //for  cell_set
    string cell_name;
    vector<int> net_vec; //紀錄一個cell擁有的net_id，不用有順序(這邊可以改成map)
};

struct net_node{    //for  net_set
    string net_name;
    vector<int> cell_vec; //紀錄一個cell擁有的net_id，不用有順序(這邊可以改成map)
};
...
struct cell_node_of_bucket_list{
	...
	int cell_id;
    bool locked_bool;
	cell_node_of_bucket_list *next_cell_node_ptr;
    cell_node_of_bucket_list *prev_cell_node_ptr;
};

struct bucket_list_node{
	int gain;	//中間等於0
	cell_node_of_bucket_list *list_ptr;
};

struct bucket_list{
	cell_node_of_bucket_list* max_gain_node_ptr;
	vector <bucket_list_node> bucket_list;
};

struct bucket_list_class{
    struct bucket_list  bucket_list_part_A;
    unordered_map<int,cell_node_of_bucket_list*> cell_id_to_node_map_part_A;

    struct bucket_list  bucket_list_part_B;
    unordered_map<int,cell_node_of_bucket_list*> cell_id_to_node_map_part_B;
};
//--------------------------------------------

struct nums_of_From_and_To_of_net{
    int nums_of_cell_in_part_A;
    int nums_of_cell_in_part_B;
};
//--------------------------------------------
...
void construct_cell_node(string cell_name,string net_name,unordered_map<int, cell_node> &cell_set){
    //先尋找這個cell_id是否有出現過
    ...
    if (it == cell_set.end()){     //沒有出現過，就先初始化該cell的node插入
        ...
        my_cell_node.net_vec.push_back(net_id);
        cell_set[cell_id]=my_cell_node;
    }
    else{ //有出現過
        it->second.net_vec.push_back(net_id);
    }
}

void construct_net_node(string cell_name,string net_name,unordered_map<int, net_node> &net_set){
    //先尋找這個net_id是否有出現過
    int cell_id=string_to_Int(cell_name);
    int net_id=string_to_Int(net_name);
    ...
    if (it == net_set.end()){     //沒有出現過，就先初始化該net的node插入
        ...
        my_net_node.cell_vec.push_back(cell_id);
        net_set[net_id]=my_net_node;
    }
    else{ //有出現過
        it->second.cell_vec.push_back(cell_id);
    }
}

void construct_net_lines_to_cell_set_and_net_set(vector<string> net_lines, unordered_map<int, cell_node>&cell_set,unordered_map<int, struct net_node>&net_set){
...
void read_file(string input_file_name, unordered_map<int, cell_node>&cell_set, unordered_map<int, net_node>&net_set){ //讀取檔案並將每一行存入 vector<string>
...
unordered_map<int,string> initialize_partition(unordered_map<int, cell_node> cell_set){
    int count=0;
...
bool determine_net_undercut_or_not(int net_id,unordered_map<int, net_node>net_set,unordered_map<int,string> patition_result){
    int nums_of_part_A_cell=0;
    int nums_of_part_B_cell=0;

    for(auto related_cell_id:net_set[net_id].cell_vec){
...
int calculate_cutsize(unordered_map<int, net_node>net_set, unordered_map<int,string> partition_result){
    int cutsize=0;
...
void show_partition_result(unordered_map<int, net_node>net_set,unordered_map<int,string> patition_result){
    for(const auto it : patition_result){
...
void find_max_num_of_net(unordered_map<int,cell_node>cell_set, int &max_gain, unordered_map<int,string> patition_result){
    max_gain=0;

    for(const auto it : patition_result){
        int cell_id=it.first;
        int net_num=cell_set[cell_id].net_vec.size();//查看該cell共有多少net
        if(max_gain<net_num)max_gain=net_num;
    }
}


void find_nums_of_net_undercut_and_not_undercut(int cell_id,unordered_map<int, cell_node>cell_set,unordered_map<int,struct net_node>net_set,unordered_map<int,string> patition_result,int &nums_of_net_not_under_cut,int &nums_of_net_under_cut){
    nums_of_net_under_cut=0;
    nums_of_net_not_under_cut=0;

    for(auto related_net_id:cell_set[cell_id].net_vec){
...
//OK
unordered_map <int, nums_of_From_and_To_of_net> calculate_From_and_To (unordered_map<int,struct net_node>net_set,unordered_map<int,string> partition_result){
    unordered_map <int, nums_of_From_and_To_of_net> From_and_To_map;
    for(auto net_it:net_set){
        int net_id=net_it.first;
        int nums_of_cell_in_part_A=0;
        int nums_of_cell_in_part_B=0;
        nums_of_From_and_To_of_net node;

        for(auto related_cell_id:net_it.second.cell_vec){
            if(partition_result[related_cell_id]=="A"){
                nums_of_cell_in_part_A++;
            }
            else{
                nums_of_cell_in_part_B++;
            }
            if(nums_of_cell_in_part_A>1&&nums_of_cell_in_part_B>1)break;
        }
        node.nums_of_cell_in_part_A=nums_of_cell_in_part_A;
        node.nums_of_cell_in_part_B=nums_of_cell_in_part_B;
        From_and_To_map[net_id]=node;
    }
    return From_and_To_map;
}

int calculate_gain_of_cell(int cell_id,unordered_map<int,cell_node>cell_set,unordered_map<int,struct net_node>net_set,unordered_map<int,string> patition_result){
    int gain=0;  //可以把所有net travel過一次就好，不會重複計算一個net的from和to
    string From_part=patition_result[cell_id];
    unordered_map <int, nums_of_From_and_To_of_net> From_and_To_map=calculate_From_and_To (net_set,patition_result);

    for(auto net_id: cell_set[cell_id].net_vec){
        int From=0;int To=0;
        if(From_part=="A"){
            From=From_and_To_map[net_id].nums_of_cell_in_part_A;
            To=From_and_To_map[net_id].nums_of_cell_in_part_B;
        }
        else{
            From=From_and_To_map[net_id].nums_of_cell_in_part_B;
            To=From_and_To_map[net_id].nums_of_cell_in_part_A;
        }
        //===for debug===
...
void insert_cell_node_to_bucket_list(int index, cell_node_of_bucket_list* my_cell_node_of_bucket_list, vector<struct bucket_list_node> &bucket_list){
    //要先查詢當前node的next node是否為空，不為空的話要往下走，走到底再插入
    //直接插在頭就好
    cell_node_of_bucket_list * node_ptr=bucket_list[index].list_ptr;
...
void resize_bucket_list(int max_gain,vector<bucket_list_node> &bucket_list){
...
void show_bucket_list(vector<bucket_list_node> bucket_list){
    for(int i=0;i<bucket_list.size();i++){
        cout<<"gain: ";
        cout<<bucket_list[i].gain<<endl;
        
        cell_node_of_bucket_list* node_ptr=bucket_list[i].list_ptr;
...
cell_node_of_bucket_list* find_max_gain_node_ptr(vector<bucket_list_node> bucket_list){
    for(int i=bucket_list.size()-1;i>=0;i--){
        cell_node_of_bucket_list* cur_node=bucket_list[i].list_ptr;
        while(1){
...
bucket_list_class initialize_bucket_list_object(unordered_map<int,cell_node>cell_set,unordered_map<int,struct net_node>net_set,unordered_map<int,string> partition_result){
    bucket_list_class my_bucket_list_class;
    ...
    for(auto it:cell_set){
        ...
        cell_node_of_bucket_list* my_cell_node_of_bucket_list_ptr=new cell_node_of_bucket_list;
...
bucket_list_class assign_bucket_list_object(bucket_list_class my_bucket_list_class, unordered_map<int,struct cell_node>cell_set,unordered_map<int,struct net_node>net_set,unordered_map<int,string> partition_result){
    bucket_list_class bucket_list_class_to_be_assigned;
    ...
    for(auto it:cell_set){
        int cell_id=it.first;
        cell_node_of_bucket_list* my_cell_node_of_bucket_list_ptr=new cell_node_of_bucket_list; //malloc
        my_cell_node_of_bucket_list_ptr->cell_id=cell_id;
        my_cell_node_of_bucket_list_ptr->locked_bool=false;
        my_cell_node_of_bucket_list_ptr->next_cell_node_ptr=NULL;
        my_cell_node_of_bucket_list_ptr->prev_cell_node_ptr=NULL;
...
void unlock_bucket_list_object(bucket_list_class &my_bucket_list_class){
    //解鎖所有cell node
...
int find_max_gain_cell_under_balance_constraint(bucket_list_class my_bucket_list_class,unordered_map<int,string> partition_result,int &cell_gain){
...
void update_max_gain_ptr_A_and_B(bucket_list_class &my_bucket_list_class){
    my_bucket_list_class.bucket_list_part_A.max_gain_node_ptr = find_max_gain_node_ptr(my_bucket_list_class.bucket_list_part_A.bucket_list);
    my_bucket_list_class.bucket_list_part_B.max_gain_node_ptr = find_max_gain_node_ptr(my_bucket_list_class.bucket_list_part_B.bucket_list);
}

void insert_node_to_tail_of_certain_gain(int new_gain,vector <bucket_list_node> &bucket_list,struct cell_node_of_bucket_list* node_ptr_to_insert){
...
void move_cell_and_update_gain(int cell_id_to_move, int new_gain, unordered_map<int,cell_node>cell_set, unordered_map<int,struct net_node>net_set, bool locked_bool,
    unordered_map<int,cell_node_of_bucket_list*> &cell_map_src, bucket_list &bucket_list_src, struct bucket_list &bucket_list_des, unordered_map<int,string> partition_result){
    cell_node_of_bucket_list* cur_node_ptr = cell_map_src[cell_id_to_move];//要被移動的cell
    ...
    if(bucket_list_src.bucket_list[index].list_ptr==cur_node_ptr){    //代表當前的node是這個Linked list的head node
        ...
        cell_node_of_bucket_list* prev_node_ptr_of_cur_node=cur_node_ptr->prev_cell_node_ptr;
...
void find_F_N_and_T_N_map_before_move(int cell_id_to_move, int critical_net_id, unordered_map<int,string> partition_result, unordered_map<int,struct net_node> net_set, 
    bucket_list_class my_bucket_list_class, unordered_map<int,int> &F_N_cell_id_to_gain_map_before_move, unordered_map<int,int> &T_N_cell_id_to_gain_map_before_move){
    
        //這邊的partition_result是移動以前的
    for(auto related_cell:net_set[critical_net_id].cell_vec){
        cell_node_of_bucket_list* related_cell_node;
...
void update_all_the_related_cell_gain(int cell_id_to_move, unordered_map<int,struct cell_node>cell_set,unordered_map<int,struct net_node>net_set,struct bucket_list_class &my_bucket_list_class, unordered_map<int,string> partition_result, int &new_gain_of_base_cell){
    ...
    for(auto critical_net_id: cell_set[cell_id_to_move].net_vec){
...
void Fiduccia_Mattheyses(unordered_map<int,struct cell_node>cell_set,unordered_map<int,struct net_node>net_set,string output_file_name,unordered_map<int,string> &partition_result,int cutsize_after_SA){
    unordered_map<int,string> cur_best_partition=partition_result;  //只存best
    unordered_map<int,string> history_best_partition=partition_result;
    //unordered_map<int,string> partition_result=cur_best_partition;

    int cur_cutsize = cutsize_after_SA;
    ...
    write_file(output_file_name, cur_cutsize,  partition_result);
    ...
    while(1){
        ...
        while(1){
            ...
            cout<<"FM: selected cell_id_to_move: "<<cell_id_to_move<<" outer_loop_counter: "<<outer_loop_counter<<endl;
            ...
            inner_loop_counter++;
            if(inner_loop_counter%3==0) write_file(output_file_name, cur_best_cutsize,  cur_best_partition);
        ...
        write_file(output_file_name, history_best_cutsize,  history_best_partition);
        outer_loop_counter++;
    }
    cout<<"final result: "<<history_best_cutsize<<endl;
}

//===Bonus Initializing with SA===
void calculate_From_and_To_for_SA(int net_id,string From_part,
    unordered_map<int,struct net_node>net_set,unordered_map<int,string> partition_result,int &From,int &To){
    From=0;
    To=0;
    for(auto related_cell_id:net_set[net_id].cell_vec){
        if(partition_result[related_cell_id]==From_part)From++;
        else To++;
        if(From>1&&To>0){
            From=-1;To=-1;
            break;
...
int calculate_gain_of_cell_for_SA(int cell_id,unordered_map<int,struct cell_node>cell_set,
    unordered_map<int,struct net_node>net_set,unordered_map<int,string> &patition_result){
    int gain=0;
    string From_part=patition_result[cell_id];
    for(auto net_id: cell_set[cell_id].net_vec){
        int From=0;int To=0;
        calculate_From_and_To_for_SA(net_id,From_part,net_set,patition_result,From,To);
        if(From==1)gain++;
        if(To==0)gain--;
    }
    return gain;
...
void construct_part_A_B_set(unordered_map<int,string> partition_result, unordered_set<int> &part_A_cell_set, unordered_set<int> &part_B_cell_set){
    for(auto it:partition_result){
        if(it.second=="A"){
            part_A_cell_set.insert(it.first);
        }
        else{
            part_B_cell_set.insert(it.first);
        }
    }
}

int getRandomFromUnorderedSet(const unordered_set<int>& uset) {
    static bool seeded = false;// 初始化隨機種子（只做一次）
    if (!seeded) {
        srand(static_cast<unsigned int>(std::time(nullptr)));
        seeded = true;
    }
    int index = std::rand() % uset.size();//取得隨機索引

    auto it = uset.begin();               //使用iterator移動到隨機位置
    std::advance(it, index);
    return *it;
}

int get_random_cell_id_from_certain_part(unordered_map<int,struct cell_node>cell_set, unordered_set<int> &part_A_cell_set, unordered_set<int> &part_B_cell_set,string &chosen_part){
    if(part_A_cell_set.size()>part_B_cell_set.size()){
        chosen_part="A";
        return getRandomFromUnorderedSet(part_A_cell_set);
    }
    else {
        chosen_part="B";
        return getRandomFromUnorderedSet(part_B_cell_set);
    }
}

int initialize_with_SA(unordered_map<int,struct cell_node>cell_set, unordered_map<int,struct net_node>net_set, unordered_map<int,string> &partition_result, double initialTemp, double coolingRate, int iterations){
    int cur_bestCutSize = calculate_cutsize(net_set,partition_result);
    unordered_set<int> part_A_cell_set; 
    unordered_set<int> part_B_cell_set;

    construct_part_A_B_set(partition_result,part_A_cell_set, part_B_cell_set);
    double temp = initialTemp;

    srand(time(0));
    for(int i=0;i<3;i++){
        for (int iter = 0; iter < 1; iter++){      //選擇一個節點進行翻轉，以當前Partition為基礎去調整
            string chosen_part="None";
            int cell_index= get_random_cell_id_from_certain_part(cell_set, part_A_cell_set, part_B_cell_set,chosen_part);
            
            int gain_of_the_selected_cell=calculate_gain_of_cell_for_SA(cell_index,cell_set,net_set,partition_result);
            cout<<"gain_of_the_selected_cell: "<<gain_of_the_selected_cell<<endl;//已確認正確

            int newCutSize=cur_bestCutSize-gain_of_the_selected_cell;            //計算新的 cut-size
            int delta=-gain_of_the_selected_cell;   // 計算接受機率

            if(delta<0){                            // 接受新解(更新)
                cur_bestCutSize = newCutSize;
                if(chosen_part=="A"){
                    part_A_cell_set.erase(cell_index);
                    part_B_cell_set.insert(cell_index);
                    partition_result[cell_index]="B";
                }
                else{
                    part_B_cell_set.erase(cell_index);
                    part_A_cell_set.insert(cell_index);
                    partition_result[cell_index]="A";
                }
            }
            temp *= coolingRate;  // 降溫
        }
    }
    cout<<"cutsize after SA: "<<cur_bestCutSize<<endl;
    return cur_bestCutSize;
}
//================================

int main(int argc, char *argv[]){
    string input_file_name = argv[1];               //讀取第一個參數作為輸入檔案名稱
    string output_file_name= argv[2];               //讀取第二個參數作為輸出檔案名稱
    unordered_map<int, cell_node> cell_set;         //以cell為單位，記錄所有cell的id以及所有相關的net
    unordered_map<int, net_node> net_set;           //以net為單位，記錄所有net的id以及所有相關的cell
    
    read_file(input_file_name,cell_set,net_set);
    //show_cell_set(cell_set);
    //show_net_set(net_set);

    unordered_map<int,string> partition_result=initialize_partition(cell_set);
    int cutsize_after_SA = initialize_with_SA(cell_set, net_set, partition_result, 10000, 0.95, 100);
    Fiduccia_Mattheyses(cell_set,net_set,output_file_name,partition_result,cutsize_after_SA);
