...
struct cell_node_of_bucket_list* find_max_gain_node_ptr(vector<struct bucket_list_node> bucket_list){
    for(int i=bucket_list.size()-1;i>=0;i--){
        ...
        while(1){
            ...
            else cur_node=cur_node->next_cell_node_ptr;
        }
    }
    return NULL;
}
...
void Fiduccia_Mattheyses(unordered_map<int,struct cell_node>cell_set,unordered_map<int,struct net_node>net_set){
    ...
    int cur_best_cutsize=calculate_cutsize(cell_set, best_partition);
    ...
    for(int i=0;i<100;i++){
        ...
        while(1){
            ...
            if(cell_id_to_move==-1)break;
                ...
                if(cur_cutsize<cur_best_cutsize){
                    ...
                    best_partition=partition_result;
                }
            }
            //partition和bucket list對不起來，
        ...
        partition_result=best_partition;//加上這行後有小bug
    }
    cout<<"final result: "<<calculate_cutsize(cell_set, partition_result)<<endl;
}
