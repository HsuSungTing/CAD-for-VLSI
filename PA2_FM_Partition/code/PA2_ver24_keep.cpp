...
bool determine_net_undercut_or_not(int net_id,unordered_map<int,struct net_node>net_set,unordered_map<int,string> patition_result){
    ...
    for(auto related_cell_id:net_set[net_id].cell_id_set){
        ...
        if(nums_of_part_A_cell!=0&&nums_of_part_B_cell)return true;
    }
...
void show_patition_result_and_cutsize(unordered_map<int,struct net_node>net_set,unordered_map<int,string> patition_result){
    for(const auto it : patition_result){
...
int find_max_gain_cell_under_balance_constraint(struct bucket_list_class my_bucket_list_class,unordered_map<int,string> partition_result,int &cell_gain){
    ...
    cout<<"can_move_from_A_bool: "<<can_move_from_A_bool<<" can_move_from_B_bool: "<<can_move_from_B_bool<<endl;
    //===============
...
void move_cell_and_update_gain(int cell_id_to_move,int base_cell_id, unordered_map<int,struct cell_node>cell_set, unordered_map<int,struct net_node>net_set, bool locked_bool,
    ...
    int old_gain=cur_node_ptr->gain;
    int new_gain=calculate_gain_of_cell(cell_id_to_move,cell_set,net_set,partition_result);
    
    cout<<"cell_id_to_move: "<<cell_id_to_move<<" old_gain: "<<old_gain<<" new_gain:"<<new_gain<<endl; 

    if(old_gain!=new_gain||cell_id_to_move==base_cell_id){  //有兩種情況要更新gain並移動
        int max_gain=(bucket_list_src.bucket_list.size()-1)/2;
        int index=max_gain+cur_node_ptr->gain;
        
        cout<<"cell_id_to_move: "<<cell_id_to_move<<" locked bool: "<<locked_bool<<endl; 
...
void construct_related_cell_map(unordered_map<int,struct cell_node>cell_set,unordered_map<int,struct net_node>net_set,unordered_map<int, unordered_set<int>> &related_cell_map){
    for(auto cell_element:cell_set){
        int cell_id=cell_element.first;
        unordered_set<int> related_cell_set;

        for(auto related_net_id:cell_set[cell_id].net_id_set){

            for(auto related_cell_id:net_set[related_net_id].cell_id_set){
                if(related_cell_id!=cell_id)related_cell_set.insert(related_cell_id);
            }
        }
        related_cell_map[cell_id]=related_cell_set;
    }
    //===for debug===
    // for(auto it:related_cell_map){
    //     cout<<"all the related cell of cell id: "<<it.first<<endl;
...
void update_all_the_related_cell_gain(int cell_id_to_move,struct bucket_list_class &my_bucket_list_class , unordered_map<int,struct cell_node>cell_set,unordered_map<int,struct net_node>net_set,unordered_map<int,string> partition_result,unordered_map<int, unordered_set<int>> related_cell_map){
    //重新計算和cell_id_to_move持有相同net的cell的gain(比對過後先找出來，再重新計算)
    unordered_set<int>related_cell_set= related_cell_map[cell_id_to_move];

    for(auto cell_id:related_cell_set){
        string cell_part_str=partition_result[cell_id];
        cout<<"base cell: "<<cell_id_to_move<<" related cell_id to be updated: "<<cell_id<<endl;   //related cell

        if(cell_part_str=="A"){//某些cell會卡住做不完
            //都是part A的cell，在part A內移動就好
            bool locked_bool=my_bucket_list_class.cell_id_to_node_map_part_A[cell_id]->locked_bool;
            if(locked_bool==false)move_cell_and_update_gain(cell_id,cell_id_to_move, cell_set,net_set,locked_bool, my_bucket_list_class.cell_id_to_node_map_part_A, my_bucket_list_class.bucket_list_part_A, my_bucket_list_class.bucket_list_part_A, partition_result);
        }
        else{
            //都是part B的cell，在part B內移動就好
            bool locked_bool=my_bucket_list_class.cell_id_to_node_map_part_B[cell_id]->locked_bool;
            if(locked_bool==false)move_cell_and_update_gain(cell_id,cell_id_to_move, cell_set,net_set,locked_bool, my_bucket_list_class.cell_id_to_node_map_part_B, my_bucket_list_class.bucket_list_part_B, my_bucket_list_class.bucket_list_part_B, partition_result);
        }
        cout<<"related cell_id done: "<<cell_id<<" "<<cell_part_str<<endl;   //related cell
        //===for debugging===
...
void move_cell_and_update_bucket_list_and_partition(int cell_id_to_move,unordered_map<int,struct cell_node>cell_set,unordered_map<int,struct net_node>net_set,struct bucket_list_class &my_bucket_list_class, unordered_map<int,string> &partition_result,unordered_map<int, unordered_set<int>> related_cell_map){
    cout<<"cell_id_to_move in move_cell_and_update_bucket_list_and_partition: "<<cell_id_to_move<<endl;
    ...
    if(partition_result[cell_id_to_move]=="A"){
        ...
        update_all_the_related_cell_gain(cell_id_to_move,my_bucket_list_class , cell_set,net_set,partition_result,related_cell_map);

        cout<<"here 333"<<endl;
        //重新計算cell_id_to_move的gain並移動該node，並上鎖(上鎖部分確定OK)，這邊的map要動，因為part A map的cell ptr會被刪除，part Bmap的cell ptr會增加
        move_cell_and_update_gain(cell_id_to_move,cell_id_to_move, cell_set, net_set,true, my_bucket_list_class.cell_id_to_node_map_part_A, my_bucket_list_class.bucket_list_part_A, my_bucket_list_class.bucket_list_part_B, partition_result);
        ...
        if(cell_node_to_move->locked_bool==true)cout<<"base cell is locked after moved"<<endl;
        if(my_bucket_list_class.cell_id_to_node_map_part_B[cell_id_to_move]->cell_id==cell_id_to_move)cout<<"base cell is in right map after moved"<<endl;
        ...
        //把cell_id從B移動到A
        cout<<"cell_id_to_move: "<<cell_id_to_move<<" in "<<partition_result[cell_id_to_move]<<endl;
        ...
        move_cell_and_update_gain(cell_id_to_move,cell_id_to_move, cell_set, net_set,true, my_bucket_list_class.cell_id_to_node_map_part_B, my_bucket_list_class.bucket_list_part_B, my_bucket_list_class.bucket_list_part_A, partition_result);
        ...
        if(cell_node_to_move->locked_bool==true)cout<<"base cell is locked after moved"<<endl;
        if(my_bucket_list_class.cell_id_to_node_map_part_A[cell_id_to_move]->cell_id==cell_id_to_move)cout<<"base cell is in right map after moved"<<endl;
...
void Fiduccia_Mattheyses(unordered_map<int,struct cell_node>cell_set,unordered_map<int,struct net_node>net_set, unordered_map<int, unordered_set<int>> related_cell_map){
    ...
    int cur_best_cutsize=calculate_cutsize(net_set, cur_best_partition);
    ...
    while(1){
        ...
        while(1){
            ...
            cout<<"FM selected bace cell: "<<cell_id_to_move<<endl;
            if(cell_id_to_move==-1)break;
                ...
                move_cell_and_update_bucket_list_and_partition(cell_id_to_move,cell_set,net_set,my_bucket_list_class, partition_result, related_cell_map);
    ...
    cout<<"final result: "<<calculate_cutsize(net_set, history_best_partition)<<endl;
}
...
int main(){
    ...
    unordered_map<int, unordered_set<int>> related_cell_map; //紀錄該和cell持有相同net的所有cell
    //會用unordered_map的原因是會需要反覆以cell name查找，且不保證cell name有序與連續

    read_file("input.txt",cell_set,net_set);
    construct_related_cell_map(cell_set,net_set,related_cell_map);//確認正確
    //show_related_cell_map(related_cell_map);
    //show_cell_set(cell_set);
    //show_net_set(net_set);
    //unordered_map<int,string> partition_result=initialize_partition(cell_set);
    Fiduccia_Mattheyses(cell_set,net_set,related_cell_map);
