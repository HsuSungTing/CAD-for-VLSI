...
//FM 沒有constraint且沒有實作出bucket list
//修改了line 192，都直接假定有通過constraint
...
bool check_if_positive_and_valid_gain_exist(unordered_map<int,struct gain_node> gain_map){
    for(auto it:gain_map){
        if(it.second.gain>0&&it.second.met_constraint_bool==true&&it.second.locked_bool==false){
            cout<<"valid cell exist"<<endl;
            return true;
        }
    }
    return false;
