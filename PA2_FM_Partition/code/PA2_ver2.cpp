...
struct gain_node{   //計算移動每個node後可以得到的cut_size、目前是否可以被移動、目前的移動方式是否合法
    bool locked_bool;
    int cutsize;
...
struct gain_node construct_gate_node(unordered_map<int,struct cell_node>cell_set,unordered_map<int,string> &patition_result,int cell_id_to_be_moved){
    ...
    if(check_constraint_of_partition(patition_result)==true){   //符合要求才需要做後續計算
        my_gain_node.met_constraint_bool=true;  
        my_gain_node.cutsize=calculate_cutsize(cell_set, patition_result);
        my_gain_node.locked_bool=false;
    }
    else{
        my_gain_node.met_constraint_bool=false;
        my_gain_node.cutsize=0;
        my_gain_node.locked_bool=false;  //不能直接鎖了，
    }
    //再把該net_id移動回來
    move_specific_net_in_partition(patition_result,cell_id_to_be_moved);
    return my_gain_node;
...
void initialize_gain_map(unordered_map<int,struct gain_node>&gain_map, unordered_map<int,struct cell_node>cell_set, unordered_map<int,string> patition_result){
    ...
    for(auto it:patition_result){
        ...
        construct_gate_node(cell_set,patition_result,cell_id_to_be_moved);
        gain_map[cell_id_to_be_moved]=my_gain_node;     //完成該gate_node
    }
}

void revise_gain_map(unordered_map<int,struct gain_node> &gain_map,unordered_map<int,struct cell_node>cell_set, unordered_map<int,string> patition_result){
    //只要被鎖定之後就不用算
...
void Fiduccia_Mattheyses_algorithm(unordered_map<int,struct cell_node>cell_set){
    ...
    while(1){
        ...
        //先檢查一次，是否可以移動任何cell來讓cutsize比cur_best_cutsize更小，如果不行就break
        while(1){
            //根據當前的gain_map找出gain_map cutsize最小且該cutsize小於cur_best_cutsize且合法 且尚未移動過的 cell(回傳id即可)，這邊寫個func A即可解決
            //如果該cell存在   1.把gain_map中該cell_id鎖上  2.更新gain_map (revise那個)  3.更新best_partition(patition_result)
            //else 當前全部都被鎖上 or 不合法，就break
        }
