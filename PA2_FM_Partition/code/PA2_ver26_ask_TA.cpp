...
struct bucket_list_class assign_bucket_list_object(struct bucket_list_class my_bucket_list_class, unordered_map<int,struct cell_node>cell_set,unordered_map<int,struct net_node>net_set,unordered_map<int,string> partition_result){
    ...
    int max_gain=0;     //找max_gain才能知道bucket list要開多大
    find_max_num_of_net(cell_set, max_gain,partition_result);
    
    //用max_gain初始化bucket list的list本身
    resize_bucket_list(max_gain,bucket_list_class_to_be_assigned.bucket_list_part_A.bucket_list);   //可以被替換，但等等
    resize_bucket_list(max_gain,bucket_list_class_to_be_assigned.bucket_list_part_B.bucket_list);   //可以被替換，但等等
    ...
    for(auto it:cell_set){
        ...
        if(partition_result[cell_id]=="A"){
            ...
            insert_cell_node_to_bucket_list(gain+max_gain,my_cell_node_of_bucket_list_ptr,bucket_list_class_to_be_assigned.bucket_list_part_A.bucket_list);
            ...
            insert_cell_node_to_bucket_list(gain+max_gain,my_cell_node_of_bucket_list_ptr,bucket_list_class_to_be_assigned.bucket_list_part_B.bucket_list);
...
void Fiduccia_Mattheyses(unordered_map<int,struct cell_node>cell_set,unordered_map<int,struct net_node>net_set){
    ...
    while(1){
        ...
        cur_best_bucket_list_class = assign_bucket_list_object(my_bucket_list_class,cell_set,net_set,cur_best_partition);
        ...
        if(cur_best_cutsize<history_best_cutsize){ //不確定這樣的做法是不是最好
            ...
            //===for debug===
            // cout<<"outer loop outer move: "<<endl;
            // cout<<"bucket list part A:"<<endl;
        ...
        //===for debug===
        // cout<<"Method 1: We Unlocked history_best_bucket_list_class "<<ct<<endl;
        // cout<<"A: "<<endl;
        // show_bucket_list(history_best_bucket_list_class.bucket_list_part_A.bucket_list);
        // cout<<"B: "<<endl;
        // show_bucket_list(history_best_bucket_list_class.bucket_list_part_B.bucket_list);

        // cout<<"Method 2: my_bucket_list_class "<<ct<<endl;
        // cout<<"A: "<<endl;
