...
//有加上提前寫檔，避免跑不完直接0分
//每跑完五次內部迴圈就寫檔一次，應該是目前最好的做法
//加上SA(Bonus)
//更新initialize bucket list

using namespace std;

struct cell_node{    //for  cell_set
    string cell_name;
    unordered_set<int> net_id_set; //改成vector!!!!
};

struct net_node{    //for  net_set
    string net_name;
    unordered_set<int> cell_id_set; //改成vector!!!!
};
...
struct bucket_list{
	struct cell_node_of_bucket_list* max_gain_node_ptr; //也可以用stl的list實現
...
//OK
unordered_map <int,struct nums_of_From_and_To_of_net> calculate_From_and_To (unordered_map<int,struct net_node>net_set,unordered_map<int,string> partition_result){
    unordered_map <int,struct nums_of_From_and_To_of_net> From_and_To_map;
    for(auto net_it:net_set){
        int net_id=net_it.first;
        int nums_of_cell_in_part_A=0;
        int nums_of_cell_in_part_B=0;
        struct nums_of_From_and_To_of_net node;

        for(auto related_cell_id:net_it.second.cell_id_set){
...
int calculate_gain_of_cell(int cell_id,unordered_map<int,struct cell_node>cell_set,unordered_map<int,struct net_node>net_set,unordered_map<int,string> patition_result){
    ...
    unordered_map <int,struct nums_of_From_and_To_of_net> From_and_To_map=calculate_From_and_To (net_set,patition_result);  //O(P)

    for(auto net_id: cell_set[cell_id].net_id_set){         //O(N of cell_id)
        int From=0;int To=0;
        if(From_part=="A"){
...
int initialize_gain_of_FM(int cell_id,unordered_map<int,struct cell_node>cell_set,unordered_map<int,struct net_node>net_set,unordered_map<int,string> patition_result, unordered_map <int,struct nums_of_From_and_To_of_net> From_and_To_map){
    int gain=0;  //可以把所有net travel過一次就好，不會重複計算一個net的from和to
    string From_part=patition_result[cell_id];
...
struct bucket_list_class initialize_bucket_list_object(unordered_map<int,struct cell_node>cell_set,unordered_map<int,struct net_node>net_set,unordered_map<int,string> partition_result){
    ...
    unordered_map <int,struct nums_of_From_and_To_of_net> From_and_To_map=calculate_From_and_To (net_set,partition_result); //O(P)

    //初始化每個cell的cell node
    for(auto it:cell_set){
        int cell_id=it.first;
        int gain=initialize_gain_of_FM(cell_id,cell_set,net_set,partition_result,From_and_To_map);
...
int initialize_with_SA(unordered_map<int,struct cell_node>cell_set, 
    unordered_map<int,struct net_node>net_set, unordered_map<int,string> &partition_result, double initialTemp, double coolingRate, int iterations){
...
int main(int argc, char *argv[]){
    ...
    //先開list存所有cell_node
    //先開list存所有net_node
    //再來unordered_map只能用第一層unordered_map<int, cell_node*> cell_set，指到list所儲存的node
    //再來unordered_map只能用第一層unordered_map<int, net_node*> net_set，指到list所儲存的net
