...
// update max gain有小問題，不能直接往後移，要把max_gain記錄下來後重找(要重找)，可以等全部更新完(所有cell都移動完再重找)
using namespace std;
...
void update_max_gain_ptr(struct cell_node_of_bucket_list* &max_gain_node_ptr,vector <struct bucket_list_node> &bucket_list){
    ...
    if(max_gain_node_ptr->next_cell_node_ptr!=NULL){
        max_gain_node_ptr=max_gain_node_ptr->next_cell_node_ptr;//如果這一格還有node，就直接往下走
    }
    else{                                      //用gain和max_gain反推index
        int max_gain=(bucket_list.size()-1)/2;
        int cur_index=max_gain+max_gain_node_ptr->gain; //計算當前node在bucket list中的index
        if(max_gain_node_ptr->next_cell_node_ptr!=NULL){
            max_gain_node_ptr=max_gain_node_ptr->next_cell_node_ptr;
        }
        else{
            cur_index--;
            while(1){
                ...
                else if(bucket_list[cur_index].list_ptr!=NULL&&bucket_list[cur_index].list_ptr->locked_bool==false){
                    max_gain_node_ptr=bucket_list[cur_index].list_ptr;      //index從大往小開始找
                    break;
                }
                else if(cur_index>=0){
                    cur_index--;
...
void move_cell_and_update_gain(int cell_id_to_move, unordered_map<int,struct cell_node>cell_set, unordered_map<int,struct net_node>net_set, bool locked_bool,
    ...
    if(prev_node_ptr_of_cur_node==cur_node_ptr){//代表當前的node是這個Linked list上唯一的node
        //cout<<"make sense 1: prev_node_ptr_of_cur_node->cell_id: "<<prev_node_ptr_of_cur_node->cell_id<<endl;
        bucket_list_src.bucket_list[index].list_ptr=cur_node_ptr->next_cell_node_ptr;
        ...
        //if(prev_node_ptr_of_cur_node->next_cell_node_ptr==cur_node_ptr)cout<<"make sense 2"<<endl;
...
void move_cell_and_update_bucket_list_and_partition(int cell_id_to_move,unordered_map<int,struct cell_node>cell_set,unordered_map<int,struct net_node>net_set,struct bucket_list_class &my_bucket_list_class, unordered_map<int,string> &partition_result){
    if(partition_result[cell_id_to_move]=="A"){
        ...
        //===for debugging===
        cout<<"max_gain_node_ptr part A: "<<my_bucket_list_class.bucket_list_part_A.max_gain_node_ptr->cell_id<<endl;
        cout<<"bucket list part A:"<<endl;
        show_bucket_list(my_bucket_list_class.bucket_list_part_A.bucket_list);

        cout<<"max_gain_node_ptr part B: "<<my_bucket_list_class.bucket_list_part_B.max_gain_node_ptr->cell_id<<endl;
        cout<<"bucket list part B:"<<endl;
        show_bucket_list(my_bucket_list_class.bucket_list_part_B.bucket_list);
        //===================
    }
