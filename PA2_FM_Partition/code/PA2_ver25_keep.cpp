...
//可以交作業了(不要管結果了，能做完真的已經很不容易了)，doubly linked list完成
//嘗試講義的做法
//我在七百行有動手腳，先這樣了
...
void move_cell_and_update_gain(int cell_id_to_move, int new_gain, unordered_map<int,struct cell_node>cell_set, unordered_map<int,struct net_node>net_set, bool locked_bool,
    ...
    //===for debug===
    for(auto it:cell_map_src){
        cout<<"cell id in map: "<<it.first<<endl;
    }
    //===============
...
void find_F_N_and_T_N_map(int cell_id_to_move, int critical_net_id, unordered_map<int,string> partition_result, unordered_map<int,struct net_node> net_set, 
...
void update_F_N_and_T_N_map(int cell_id_to_move, unordered_map<int,int> F_N_cell_id_to_gain_map_before_move, unordered_map<int,int> T_N_cell_id_to_gain_map_before_move, unordered_map<int,int> &F_N_cell_id_to_gain_map_after_move, unordered_map<int,int> &T_N_cell_id_to_gain_map_after_move){
...
void  update_all_updated_cell_id_to_gain_map(int cell_id_to_move,int critical_net_id, unordered_map<int,struct cell_node>cell_set,unordered_map<int,struct net_node>net_set,struct bucket_list_class my_bucket_list_class, unordered_map<int,string> partition_result,unordered_set<int> &all_updated_cell_id_to_gain_set){
    ...
    find_F_N_and_T_N_map(cell_id_to_move, critical_net_id, partition_result, net_set, my_bucket_list_class, F_N_cell_id_to_gain_map_before_move, T_N_cell_id_to_gain_map_before_move);

    if(T_N_cell_id_to_gain_map_before_move.size()==1){
        for(auto it:T_N_cell_id_to_gain_map_before_move){
            all_updated_cell_id_to_gain_set.insert(it.first);
        }
    }
    else if(T_N_cell_id_to_gain_map_before_move.size()==0){
    ...
    update_F_N_and_T_N_map(cell_id_to_move,F_N_cell_id_to_gain_map_before_move, T_N_cell_id_to_gain_map_before_move, F_N_cell_id_to_gain_map_after_move, T_N_cell_id_to_gain_map_after_move);
    
    if(F_N_cell_id_to_gain_map_after_move.size()==1){
        for(auto it:F_N_cell_id_to_gain_map_after_move){
            all_updated_cell_id_to_gain_set.insert(it.first);
        }
    }
    else if(F_N_cell_id_to_gain_map_after_move.size()==0){
...
void update_all_the_related_cell_gain(int cell_id_to_move, unordered_map<int,struct cell_node>cell_set,unordered_map<int,struct net_node>net_set,struct bucket_list_class &my_bucket_list_class, unordered_map<int,string> partition_result){
    //在這邊會真的移動related cell在bucket list中的node(在同個bucket list中移動)
    unordered_set<int> all_updated_cell_id_to_gain_set;
    for(auto critical_net_id: cell_set[cell_id_to_move].net_id_set){
        update_all_updated_cell_id_to_gain_map(cell_id_to_move,critical_net_id, cell_set,net_set,my_bucket_list_class, partition_result,all_updated_cell_id_to_gain_set);
    }
    cout<<"all_updated_cell_id_to_gain_map.size(): "<<all_updated_cell_id_to_gain_set.size()<<endl;
    //更新all_updated_cell_id_to_gain_map的所有cell成對應的gain(要移動)
    for(auto it:all_updated_cell_id_to_gain_set){
        int cell_id=it;
        ...
        int new_gain=correct_new_gain;

        cout<<"cell_id: "<<cell_id<<" correct new_gain: "<<correct_new_gain<<endl;
            
        if(partition_result[cell_id_to_move]=="A")partition_result[cell_id_to_move]="B";
        else partition_result[cell_id_to_move]="A";

        if(cell_id!=cell_id_to_move){
            if(partition_result[cell_id]=="A"){
                cout<<"cell_id to update: "<<cell_id<<endl;
                if(my_bucket_list_class.cell_id_to_node_map_part_A[cell_id]==NULL)cout<<"my_bucket_list_class.cell_id_to_node_map_part_A[cell_id] is NULL"<<endl;
                    
                bool locked_bool=my_bucket_list_class.cell_id_to_node_map_part_A[cell_id]->locked_bool;
                //if(locked_bool==false)move_cell_and_update_gain(cell_id, new_gain, cell_set,net_set,locked_bool, my_bucket_list_class.cell_id_to_node_map_part_A, my_bucket_list_class.bucket_list_part_A, my_bucket_list_class.bucket_list_part_A, partition_result);
                ...
                cout<<"cell_id to update: "<<cell_id<<endl;
                if(my_bucket_list_class.cell_id_to_node_map_part_B[cell_id]==NULL)cout<<"my_bucket_list_class.cell_id_to_node_map_part_B[cell_id] is NULL"<<endl;

                bool locked_bool=my_bucket_list_class.cell_id_to_node_map_part_B[cell_id]->locked_bool;
                //if(locked_bool==false)move_cell_and_update_gain(cell_id, new_gain,cell_set,net_set,locked_bool, my_bucket_list_class.cell_id_to_node_map_part_B, my_bucket_list_class.bucket_list_part_B, my_bucket_list_class.bucket_list_part_B, partition_result);
...
void move_cell_and_update_bucket_list_and_partition(int cell_id_to_move,unordered_map<int,struct cell_node>cell_set,unordered_map<int,struct net_node>net_set,struct bucket_list_class &my_bucket_list_class, unordered_map<int,string> &partition_result){
    if(partition_result[cell_id_to_move]=="A"){
        ...
        update_all_the_related_cell_gain(cell_id_to_move,cell_set,net_set,my_bucket_list_class, partition_result);
...
void Fiduccia_Mattheyses(unordered_map<int,struct cell_node>cell_set,unordered_map<int,struct net_node>net_set){
    ...
    struct bucket_list_class cur_best_bucket_list_class = my_bucket_list_class;
    struct bucket_list_class history_best_bucket_list_class = my_bucket_list_class;
    ...
    while(1){
        ...
        cur_best_bucket_list_class = my_bucket_list_class;
        ...
        while(1){
            ...
            if(cell_id_to_move==-1||gain_of_cell_id_to_move<0){
                break;
            }
            else{
        ...
        if(cur_best_cutsize<history_best_cutsize){ //不確定這樣的做法是不是最好
            ...
            history_best_bucket_list_class = cur_best_bucket_list_class;
        }
        else {
            break;
        }
        ...
        unlock_bucket_list_object(history_best_bucket_list_class);
        my_bucket_list_class = history_best_bucket_list_class;
    ...
    write_file("output.txt", calculate_cutsize(net_set, history_best_partition),  history_best_partition);
