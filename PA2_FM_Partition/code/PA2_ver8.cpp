...
bool check_if_positive_and_valid_gain_exist(unordered_map<int,struct gain_node> gain_map){
    for(auto it:gain_map){
        if(it.second.gain>0&&it.second.met_constraint_bool==true){
            cout<<"valid cell exist, cell id: "<<it.first<<endl;
            return true;
        }
    }
    return false;
...
int find_the_unlocked_max_gain_cell_without_constraint(unordered_map<int,struct gain_node> gain_map){
    int cell_id=-1;
    int cur_best_gain=INT_MIN;
    for(auto it:gain_map){
        if(it.second.locked_bool==false){  //沒有鎖上的cell就算
...
void Fiduccia_Mattheyses_algorithm(unordered_map<int,struct cell_node>cell_set){
    ...
    unordered_map<int,string> patition_result=initialize_partition(cell_set);
    int limit=5;//如果連續五次都沒有更好的cutsize就終止外部迴圈
    ...
    while(1){
        ...
        while(1){
            ...
            int cell_id_to_be_moved=find_the_unlocked_max_gain_cell_without_constraint(gain_map);
            if(cell_id_to_be_moved==-1)break;                                             //當前全部cell都被鎖上，就break
            else{                                                                         //如果該cell id存在      
