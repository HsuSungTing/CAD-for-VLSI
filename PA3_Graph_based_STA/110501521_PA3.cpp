...
//開始處理step 3，目前插值正確
//把gate_map換成gate_vec
//目前邏輯上的做法就是在對底層的資料結構存實體，然後dependency都用Poinetr建構
...
struct gate_node {
    ...
    vector<wire_node*> output_wire;  //依然維持vector，為了快速遍歷
};

struct RTL_obj {
    vector<gate_node> gate_vec;  //改成list<gate_node> gate_vec
    //加上list<wire_node> net_vec
    unordered_map<string, wire_node> net_map; //unordered_map<string, wire_node*> net_map;
    vector<wire_node> net_vec;
...
string get_module_name(string line){
    ...
    split_by_outer_parentheses(line,part_1,part_2);
    vector<string>result=split_by_delimiter(part_1,' ');
    return result[1];
}

vector<string> read_RTL(const string filename,string &module_name){    //切comment的部分我真的寫得蠻好的
    ...
    while (getline(file, line)){
        ...
        //cout<<"line: "<<line<<endl;
        //cout<<"head_slash_star_bool: "<<head_slash_star_bool<<" end_star_slash_bool: "<<end_star_slash_bool<<endl;
        if(head_slash_star_bool==true){
            comment_mode=true;
        }
        //cout<<"comment_mode: "<<comment_mode<<endl;

        string first_token=get_first_token(line);
        if(comment_mode==false&&check_two_slash(line)==false&&is_blank_line(line)==false){
            if(first_token=="module" || first_token=="input"||first_token=="output"||first_token=="wire"||first_token=="NANDX1" || first_token=="INVX1" || first_token=="NOR2X1" || first_token=="endmodule"){
    ...
    module_name=get_module_name(lines[0]);
    //===for debug===
...
vector<string> wire_parser(const string& line) {
    vector<string> result;
    size_t first_non_space = line.find_first_not_of(' ');//找到第一個非空白字元位置
    if (first_non_space == string::npos) return result;

    string trimmed_line = line.substr(first_non_space);

    size_t space_pos = trimmed_line.find(' ');      //找第一個空格以跳過關鍵字 (input/wire/...)
    if (space_pos == string::npos) return result;

    string part_2 = trimmed_line.substr(space_pos + 1);     //取得變數區段
    part_2.erase(remove_if(part_2.begin(), part_2.end(), ::isspace), part_2.end());//去除所有空白

    if (!part_2.empty() && part_2.back() == ';') {//移除最後的分號
        part_2.pop_back();
    }
    stringstream ss(part_2);
    string token;
    while (getline(ss, token, ',')) {   //依','切割成token
        if (!token.empty()) {
...
void parse_wire_and_bulid_map(string input, unordered_map<string, wire_node>& wire_map, vector<wire_node*>& vec) {
    ...
    for (int i = 0; i < results.size(); i++) {
        ...
        wire_node node;         //因為在初始化wire_map，所以要用wire_node的實體
        node.name = results[i];
        node.start_time = 0;
        node.input_gate_transition_time = 0;

        //插入map並取得reference
        auto insert_result = wire_map.insert({node.name, node});
        wire_node& ref = insert_result.first->second;
        vec.push_back(&ref);  //存的是map中實體的位址
        //===for debug===
...
void gate_parser(gate_node &node, vector<string>part_2_result, unordered_map<string,wire_node> &wire_map){
    ...
    for(int i=0;i<part_2_result.size();i++){
        ...
        //===for debug===
        //cout<<"wire name: "<<wire_map[wire].name<<endl;
        //===============
        if(signal=="A1"||signal=="I"){
            node.input_wire[0]= &wire_map[wire];//要查到再放進來
            //===for debug===
            //cout<<"wire name: "<<node.input_wire[0]->name<<endl;
            //===============
        }
        else if(signal=="A2"){
            node.input_wire[1]= &wire_map[wire];//要查到再放進來
            //===for debug===
            //cout<<"wire name: "<<node.input_wire[1]->name<<endl;
            //===============
        }
        else if(signal=="ZN"){
            node.output_wire[0]= &wire_map[wire];//要查到再放進來
            //===for debug===
            //cout<<"wire name: "<<node.output_wire[0]->name<<endl;
            //===============
        }
    }
}

void parse_gate_and_bulid_map(string input, 
    vector<gate_node>&gate_vec, unordered_map<string, wire_node> &wire_map){
    ...
    gate_node node;  //gate_vec也是直接存實體
    node.name = part_1_result[1];
    node.id = extract_int_from_string(node.name);
    ...
    part_2=removeSpaces(part_2);
    vector<string> part_2_result = split_by_delimiter(part_2, ',');
    gate_parser(node, part_2_result, wire_map);
    gate_vec.push_back(node);
    //===for debug===  
    // cout<<"node: "<<node.name<<endl;  //這邊確定正確，node本身正確
    // for(int j=0;j<node.input_wire.size();j++){
    //     cout<<"input wire: "<<node.input_wire[j]->name<<endl;
    // }
    // cout<<"output wire: "<<node.output_wire[0]->name<<endl;
    //===============
}

void bulid_net_dependency(struct RTL_obj &RTL_object){
    for(int i=0;i<RTL_object.gate_vec.size();i++){
        //你是我的input_wire，我就是你的output gate

        //===for debug===
        // cout<<"RTL_object.gate_vec[i]: "<<RTL_object.gate_vec[i].name<<endl;
        // for(int j=0;j<RTL_object.gate_vec[i].input_wire.size();j++){
        //     cout<<"input wire: "<<RTL_object.gate_vec[i].input_wire[j]->name<<endl;
        // }
        // cout<<"output wire: "<<RTL_object.gate_vec[i].output_wire[0]->name<<endl;
        //===============

        for(int j=0;j<RTL_object.gate_vec[i].input_wire.size();j++){
            RTL_object.gate_vec[i].input_wire[j]->output_gate.push_back(&RTL_object.gate_vec[i]);
        }
        //你是我的output_wire，我就是你的input gate
        RTL_object.gate_vec[i].output_wire[0]->input_gate.push_back(&RTL_object.gate_vec[i]);
    }
    //=====for debug=====
...
void bulid_gate_dependency(struct RTL_obj &RTL_object, vector<gate_node*> &ready_gate){
    for(int i=0;i<RTL_object.gate_vec.size();i++){
        gate_node* gate=&RTL_object.gate_vec[i];  //因為這邊希望call by address
        for(int j=0;j<gate->output_wire.size();j++){
            for(int k=0;k<gate->output_wire[j]->output_gate.size();k++){
                gate->output_gate.push_back(gate->output_wire[j]->output_gate[k]);
            }
        }
        for(int i=0;i<gate->input_wire.size();i++){
            for(int k=0;k<gate->input_wire[i]->input_gate.size();k++){
                gate->input_gate.push_back(gate->input_wire[i]->input_gate[k]);
            }
        }
        if(gate->input_gate.size()==0){
            ready_gate.push_back(gate);
        }
        gate->in_degree=gate->input_gate.size();
    }
    //=====for debug=====
    // cout<<"Gate Dependency: "<<endl;
    // for(auto it:RTL_object.gate_vec){
    //     cout<<"gate_name: "<<it.name<<endl;
    //     cout<<"input_gate: "<<endl;
    //     for(int i=0;i<it.input_gate.size();i++){
    //         cout<<it.input_gate[i]->name<<endl;
    //     }
    //     cout<<"output_gate: "<<endl;
    //     for(int i=0;i<it.output_gate.size();i++){
    //         cout<<it.output_gate[i]->name<<endl;
    //     }
    // }
    //===================
}
...
void build_RTL_object(struct RTL_obj &RTL_object, vector<string> RTL_lines, vector<gate_node*> &ready_gate){
    for(int i=1;i<RTL_lines.size();i++){ //確定正確
        ...
        if(first_token=="input"){
            ...
            parse_gate_and_bulid_map(RTL_lines[i], RTL_object.gate_vec, RTL_object.net_map);
        }
    }
    //===for debug===
    for(int i=0;i<RTL_object.gate_vec.size();i++){
        cout<<"gate name: "<<RTL_object.gate_vec[i].name<<endl;

        // for(int j=0;j<RTL_object.gate_vec[i].input_wire.size();j++){
        //     cout<<"input_wire name: "<<RTL_object.gate_vec[i].input_wire[j]->name<<endl;
        // }
        cout<<"output_wire name: "<<RTL_object.gate_vec[i].output_wire[0]->name<<endl;
    }
    //===============
...
double find_input_capacitance(wire_node* net, struct liberty_file file){
    ...
    //return roundToSixDecimalPlaces(input_capacitance);
    return input_capacitance;
}

void find_load_capacitance(struct RTL_obj &RTL_object, struct liberty_file file){
    for(int i=0;i<RTL_object.gate_vec.size();i++){   //之後可以轉成vector，因為不需要用gate名稱去查找了
        gate_node* gate=&RTL_object.gate_vec[i];
...
string make_path_string(vector<wire_node*> path){
    string str=" = "+formatToSixDecimalPlaces(path[0]->start_time)+", the path is: ";
    for(int i=path.size()-1;i>=0;i--){
        if(i==0)str+=path[i]->name;
        else str+=(path[i]->name+" -> ");
    }
    return str;
}

vector<gate_node> sort_by_delay(const vector<gate_node> original_vector){
    vector<gate_node> sorted_vector=original_vector;
    sort(sorted_vector.begin(),sorted_vector.end(), [](const gate_node a, const gate_node b){
        if(a.propagation_delay != b.propagation_delay){
...
vector<gate_node> sort_by_capcitance(const vector<gate_node> original_vector){
    vector<gate_node> sorted_vector=original_vector;
    sort(sorted_vector.begin(),sorted_vector.end(), [](const gate_node a, const gate_node b){
        if(a.load_capacitance != b.load_capacitance){
            return a.load_capacitance > b.load_capacitance;
        } 
        else{
            return a.id<b.id;
        }
...
void step1_write_file(struct RTL_obj RTL_object, string module_name){
    string file_name="Load_110501521_"+module_name+".txt";
    ofstream outfile(file_name);  // 開啟檔案
    vector<gate_node> result=sort_by_capcitance(RTL_object.gate_vec);
    for(int i=0;i<result.size();i++){
        outfile<<result[i].name<<" "<<formatToSixDecimalPlaces(roundToSixDecimalPlaces(result[i].load_capacitance))<<endl;
    }
    outfile.close();     // 關閉檔案
}

void step2_write_file(struct RTL_obj RTL_object, string module_name){
    string file_name="Delay_110501521_"+module_name+".txt";
    ofstream outfile(file_name);  // 開啟檔案
    vector<gate_node> result=sort_by_delay(RTL_object.gate_vec);
    for(int i=0;i<result.size();i++){
        outfile<<result[i].name<<" "<<result[i].output_situation<<" "<<formatToSixDecimalPlaces(result[i].propagation_delay)<<" "<<formatToSixDecimalPlaces(result[i].output_transition_time)<<endl;
    }
    outfile.close();     // 關閉檔案
}

void step3_write_file(struct RTL_obj RTL_object, string module_name){
    string file_name="Path_110501521_"+module_name+".txt";
    ofstream outfile(file_name);  // 開啟檔案
    if (!outfile) {
        cout<<"can't open "<<file_name<< endl;
        return;
    }
    outfile<<"Longest delay"+make_path_string(RTL_object.longest_path)<<endl;  // 寫入內容
    outfile<<"Shortest delay"+make_path_string(RTL_object.shortest_path)<<endl;  // 寫入內容
    outfile.close();     // 關閉檔案
}

void write_file(struct RTL_obj RTL_object,string module_name){
    step1_write_file(RTL_object, module_name);
    step2_write_file(RTL_object, module_name);
    step3_write_file(RTL_object,module_name);
}

void top_function(string netlist_name, string test_lib_name){
    string module_name;
    vector<string> RTL_lines=read_RTL(netlist_name,module_name);
    //===for debug==
    ...
    find_load_capacitance(RTL_object, file);//step 1
    topoligical_ordering(RTL_object, ready_gate, file);//step 2
    find_longest_and_shortest_path(RTL_object);//step 3

    write_file(RTL_object,module_name);
}

int main(int argc, char *argv[]){
    string netlist_name = argv[1];             
    string test_lib_name= argv[2];               
    top_function(netlist_name, test_lib_name);
    return 0;
}