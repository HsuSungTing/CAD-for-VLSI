...
struct gate_node{
    ...
    double longest_delay; //為了step 3
    double shortest_delay;//為了step 3
...
void update_output_wire_of_gate(gate_node* node){//更新gate的output wire
...
double find_input_transition_time(gate_node* node){
...
void find_output_transition_and_delay(gate_node* node,struct liberty_file file){
    double input_transition=find_input_transition_time(node);
    ...
    update_output_wire_of_gate(node);
    //===for debug===
    cout<<"gate name: "<<node->name<<" propagation_delay: "<<node->propagation_delay<<" coutput_transition_time: "<<node->output_transition_time<<" output_situation: "<<node->output_situation<<endl;
...
void topoligical_ordering(struct RTL_obj RTL_object, vector<gate_node*> &ready_gate, struct liberty_file file){
    ...
    while(gate_queue.empty()==false){
        ...
        //cout<<"cur gate: "<<gate->name<<endl;
        for(int i=0;i<gate->output_gate.size();i++){
            gate->output_gate[i]->in_degree--;
            //cout<<"gate name: "<<gate->output_gate[i]->name<<" indegree after decreasing: "<<gate->output_gate[i]->in_degree<<endl;
            if(gate->output_gate[i]->in_degree==0){
                gate_queue.push(gate->output_gate[i]);
                //cout<<gate->output_gate[i]->name<<" is pushed"<<endl;
            }
        }
    }
}
