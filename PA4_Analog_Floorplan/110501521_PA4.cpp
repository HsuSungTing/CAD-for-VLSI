...
//在gpt的幫助下修改正確
//試著加上block size的filter，去除除了第一個之外明顯不優的解
//多加了一層block size的過濾，要和110501521_PA4.cpp PA4_ver12.cpp比較到底哪個好
...
vector<node> initialize_node_vec(placement &my_placement,string input_file){
    ...
    for(int i=0;i<lines.size();i++){
        ...
        //===為了過濾明顯不優的解===
        double width_max,width_min,height_max,height_min;
        //========================
        for(int i=1;i<results.size();i++){
            vector<string> values=splitInsideParentheses(results[i]);
            double cur_width=stod(values[0]);
            double cur_height=stod(values[1]);
            if(i==1){   //第一組一定要採納(無論好不好)
                width_max=cur_width;  width_min=cur_width;
                height_max=cur_height;height_min=cur_height;
            }
            else if(cur_width>=width_max&&cur_height>=height_min){ 
                continue; //skip 目前的長寬是明顯不優的解
            }
            else if(cur_height>=height_max&&cur_width>=width_min){ 
                continue; //skip 目前的長寬是明顯不優的解
            }
            else if(cur_height<=height_min&&cur_width<=width_min){
                //目前解是明顯最優選項
                my_node.candidate.clear(); my_node.candidate.resize(0);
                width_max=cur_width;  width_min=cur_width;
                height_max=cur_height;height_min=cur_height;
            }
            else{
                width_max=max(width_max,cur_width);   width_min=min(width_min,cur_width);
                height_max=max(height_max,cur_height);height_min=min(height_min,cur_height);
            }
...
void resize_random_node(placement &cur_placement){
    ...
    if(cur_placement.node_vec[selected_idx].candidate.size()==1)return; //只嘗試五次就放棄
...
placement SA_placement(placement &my_placement,string output_file_name){
    ...
    for(int i=0;i<10000;i++){
        ...
        if((delta<0)||(exp(-delta / temp) > ((double)rand() / RAND_MAX))){//接受新解
            ...
            //===for debug===
            std::ofstream ofs("accepted_costs.txt", std::ios::app); // 附加模式
            if(ofs.is_open()){
                ofs<< "Iteration "<< i<<": "<<cur_placement.cost<<std::endl;
                ofs.close();
            } 
            else{
                std::cerr << "Error: Could not open accepted_costs.txt for writing." << std::endl;
            }
            //===============
        ...
        temp*=cooling_rate;
        if(i%500==0&&i>0){          
            write_file(cur_placement,output_file_name);//每SA 500次寫檔一次
        }
    }
...
int main(int argc, char *argv[]){
    ...
    //     cout<<"block: "<<my_placement.node_vec[i].name<<" id: "<<my_placement.node_vec[i].id<<endl;
    //     cout<<"all size: "<<endl;
    //     for(int j=0;j<my_placement.node_vec[i].candidate.size();j++){
    //         cout<<"width: "<<my_placement.node_vec[i].candidate[j].width<<" ";
    //         cout<<"height: "<<my_placement.node_vec[i].candidate[j].height<<endl;
    //     }
    // }
    //===============
    sort_node_by_area(my_placement.node_vec);   //有sort還是比較好，尤其是形狀差異越大的case

    //step 2:建立初始解的placement(確定正確，我手畫過也用python比對過)，建一個tree(太寬就往上擺)
    ...
    write_file(my_placement,output_file_name);  //初始化後先寫檔一次
    
    //step 3 以SA找尋更好的解
    
    placement cur_placement=SA_placement(my_placement,output_file_name);
