...
node_ptr make_Btree(const vector<node_ptr> &node_vec, vector<vector<height_info>>& height_vec,
    double&up_bound,double& right_bound,double block_area_sum){
    if (node_vec.empty()) return nullptr;
    //===for debug===
    //vector<vector<height_info>> new_height_vec;
    //===============
    ...
    while(!stk.empty()){
        ...
        node_ptr* prev_node_link= get<3>(s);
...
void swap_two_random_node(vector<node_ptr> &node_vec){
    int n=node_vec.size();
...
void resize_random_node(placement &cur_placement){
    ...
    node_ptr node=cur_placement.node_vec[selected_idx];
    int prev_idx=node->candidate_idx;
    //扣除舊面積
    cur_placement.block_area_sum-= node->candidate[prev_idx].height * node->candidate[prev_idx].width;
    int cur_idx=rand()%node->candidate.size();
    while(cur_idx==prev_idx){           //確保選定的idx和原本不同
        cur_idx=rand()%node->candidate.size();              
    }
    node->candidate_idx=cur_idx;
    //加上新面積
    cur_placement.block_area_sum+=node->candidate[cur_idx].height* node->candidate[cur_idx].width;
}
...
void SA_placement(const placement &my_placement){
    placement cur_placement;   //紀錄目前最好的cost
    ...
    for(int i=0;i<1000;i++){
        ...
        // }
        //===============
        Perturb_node_vec(cur_placement);
        //===for debug===
        ...
        clean_placement(cur_placement);
        cur_placement.root= make_Btree(cur_placement.node_vec, cur_placement.height_vec,cur_placement.up_bound,cur_placement.right_bound,cur_placement.block_area_sum);
        //===for debug===
        //cout<<"here 5"<<endl;
        //===============
        find_centroid(cur_placement);
        find_INL(cur_placement);
        
        //step 2:計算cost，如果cost較低 or 抽到特定機率就把cur_placement更新
        cout<<"cur cost: "<<find_cost(cur_placement)<<endl;
    }
