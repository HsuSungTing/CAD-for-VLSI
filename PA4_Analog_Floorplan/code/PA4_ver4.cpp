...
void SA_placement(const placement &my_placement){
    ...
    for(int i=0;i<1000;i++){
        ...
        double xk_sum=0,yk_sum=0;                   //建一個tree(太寬就往上擺)
        clean_placement(cur_placement);
        cur_placement.root= make_Btree(cur_placement.node_vec, cur_placement.height_vec,cur_placement.up_bound,cur_placement.right_bound,xk_sum,yk_sum,cur_placement.block_area_sum);
        //===for debug===
        //cout<<"here 5"<<endl;
        //===============
        find_centroid(xk_sum,yk_sum,cur_placement);
