...
vector<node_ptr> initialization(placement &my_placement){
    ...
    my_placement.block_area_sum+=M0->candidate[0].height*M0->candidate[0].width;
    ...
    my_placement.block_area_sum+=M1->candidate[0].height*M1->candidate[0].width;
    ...
    my_placement.block_area_sum+=M2->candidate[0].height*M2->candidate[0].width;
    ...
    my_placement.block_area_sum+=M3->candidate[0].height*M3->candidate[0].width;
    node_vec.push_back(M3);

    node_ptr M4=make_shared<node>();
    M4->id=4; M4->name="MM4"; M4->candidate_idx=0; M4->candidate.push_back(build_macro(4.99, 5.54, 4, 1));
    my_placement.block_area_sum+=M4->candidate[0].height*M4->candidate[0].width;
...
//         my_placement.block_area_sum+= n->candidate[0].height * n->candidate[0].width;
...
node_ptr make_Btree(vector<node_ptr>node_vec,vector<vector<height_info>>& height_vec,double& up_bound, double& right_bound, double& xk_sum, double& yk_sum, double block_area_sum) {
    ...
    //===for debug===
    cout<<"block_area_sum: "<<block_area_sum<<" sqrt(block_area_sum): "<<sqrt(block_area_sum)<<endl;
    //===============
...
int main(){
    ...
    my_placement.node_vec= initialization(my_placement);
