...
void resize_random_node(vector<node_ptr> &node_vec){
    int n=node_vec.size();
    int selected_idx=0;
    for(int i=0;i<n;i++){
        selected_idx=rand()%n;
        if(node_vec[selected_idx]->candidate.size()>1)break;//確保選定的node有不只一個candidate
    }
    node_ptr node=node_vec[selected_idx];
    int previous_idx=node->candidate_idx;
    int cur_idx=rand()%node->candidate.size();

    while(cur_idx==previous_idx){
        cur_idx=rand()%node->candidate.size();              //確保選定的idx和原本不同
    }
...
void Perturb_node_vec(placement &cur_placement){     //80%機率swap node，20%機率改長寬
    ...
    if(r<80){
        ...
        resize_random_node(cur_placement.node_vec);//目前正確
    }
}
...
int main(int argc, char *argv[]){
    ...
    //copy_placement(my_placement,new_placement);
    //show_placement(new_placement);
    //===============
