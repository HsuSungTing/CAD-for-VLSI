...
vector<node_ptr> initialization() {
    ...
    random_device rd;// 使用 C++ <random> 隨機產生器
    mt19937 gen(rd());
...
node_ptr make_Btree(vector<node_ptr>node_vec,int idx, double cur_x, int cur_level, double area, vector<vector<height_info>>&height_vec,double &up_bound,double &right_bound,double &xk_sum,double &yk_sum){
    ...
    if(idx<node_vec.size()){
        ...
        cur_node->xk=cur_x+node_width/2;        //計算質心
        cur_node->yk=cur_node->y+node_height/2; //計算質心
        xk_sum+=cur_node->xk;
        yk_sum+=cur_node->yk;
        ...
        if(cur_x > sqrt(area)){//如果目前擺放cell的位置已經過於右邊
            //往上擺
            cur_node->right=make_Btree(node_vec,idx+1, 0,cur_level+1,area+node_width*node_height,height_vec,up_bound,right_bound,xk_sum,yk_sum);
        }
        else{ 
            //往右擺
            cur_node->left=make_Btree(node_vec,idx+1,cur_x+node_width,cur_level,area+node_width*node_height,height_vec,up_bound,right_bound,xk_sum,yk_sum);
        }
        return cur_node;
    }
    return nullptr;
...
int main(){
    ...
    my_placement.root = make_Btree(my_placement.node_vec, 0, 0, 0, 0.0, my_placement.height_vec,my_placement.up_bound,my_placement.right_bound,xk_sum,yk_sum);
