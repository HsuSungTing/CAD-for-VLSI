...
bool check_sig_match(sig_match match){
    ...
    for(int j=0;j<new_out_comb.size();j++){
        ...
        if(isSameAST(old_out_comb[j]->logic_tree.root, new_out_comb[j]->logic_tree.root)==false){
            ans=false;//只要有一組output對不到，就算沒有對到
            break;
        }
    }
...
void find_sig_match(match_info &info){
    ...
    for(int i=0;i<info.class_map.size();i++){
        vector<sig_match> possible_match=make_all_sig_match(info.class_map[i]);
